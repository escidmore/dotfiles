...
define_undefined_git_prompt_colors() {
  if [ -z ${GIT_PROMPT_THEME_NAME+x} ]; then GIT_PROMPT_THEME_NAME="Custom"; fi
  ...
  if [ -z ${GIT_PROMPT_START_USER+x} ]; then 
...
if [[ "${GIT_PROMPT_THEME}" == "Custom" && "${GIT_PROMPT_THEME_NAME}" != "Custom" ]]; then
  define_helpers
  define_undefined_git_prompt_colors
fi
